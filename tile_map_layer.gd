	...
	for x in range (mapSpace):
		...
		for y in range (height, 20):
			if y == height:
				var rand = randf()
				if rand < 0.33: 
					set_cell(Vector2i(x, y-8), 2, Vector2i(1, 8), 0)
				elif rand < 0.66:
					set_cell(Vector2i(x, y-8), 2, Vector2i(2, 8), 0)
				else: 
					set_cell(Vector2i(x, y-8), 2, Vector2i(3, 8), 0)
			else:
				set_cell(Vector2i(x, y-8), 2, Vector2i(4, 8), 0)
			#if (randf() < 0.25):
