...
func mars_gen():
	for x in range (mapSpace):
		...
		for y in range (height, 20):
			if (randf() < 0.25):
				set_cell(Vector2i(x, y-8), 1, Vector2i(0, 4), 0)
			elif (randf() < 0.5):
				set_cell(Vector2i(x, y-8), 1, Vector2i(1, 4), 0)
			elif (randf() <0.75):
				set_cell(Vector2i(x, y-8), 1, Vector2i(2, 4), 0)
			else:
				set_cell(Vector2i(x, y-8), 1, Vector2i(3, 4), 0)
			if x == mapSpace - 6:
				var goalpost_scene = preload("res://goalpost.tscn")
				var goalpost = goalpost_scene.instantiate()
				get_parent().add_child(goalpost)
...
func jupiter_gen():
	for x in range (mapSpace):
		...
		for y in range (height, 20):
			if (randf() < 0.25):
				set_cell(Vector2i(x, y-8), 2, Vector2i(0, 5), 0)
			elif (randf() < 0.5):
				set_cell(Vector2i(x, y-8), 2, Vector2i(1, 5), 0)
			elif (randf() <0.75):
				set_cell(Vector2i(x, y-8), 2, Vector2i(2, 5), 0)
			else:
				set_cell(Vector2i(x, y-8), 2, Vector2i(3, 5), 0)
			if x == mapSpace - 6:
				var goalpost_scene = preload("res://goalpost.tscn")
				var goalpost = goalpost_scene.instantiate()
				get_parent().add_child(goalpost)
