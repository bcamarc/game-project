...
@onready var body_area: Area2D = get_node_or_null("body_area") as Area2D
@onready var body_shape: CollisionShape2D = get_node_or_null("body_area/CollisionShape2D") as CollisionShape2D
...
func _physics_process(delta: float) -> void:
	...
	# phase transition
	var health_frac := float(health) / float(max_health)
	...
	state_timer -= delta
	match state:
		BossState.IDLE, BossState.PHASE1, BossState.PHASE2:
			velocity.x = move_toward(velocity.x, 0.0, walk_speed)
			_play_animation("idle")
			if state_timer <= 0.0:
...
func _process_summon() -> void:
	...
	if state_timer <= 0.0:
		if summon_scene != null:
			...
			# connect cleanup so boss knows when summon leaves
			if s.has_method("connect"):
				s.connect("tree_exited", Callable(self, "_on_summon_removed"), [s])
			summons.append(s)
			summon_timer = summon_cooldown
		state = BossState.IDLE
		state_timer = 1.0
...
func _on_summon_removed(node) -> void:
	if summons.has(node):
		summons.erase(node)
